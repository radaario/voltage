...
export async function createJobNotification(job, jobStatus) {
    ...
    if (job?.notification?.try && parseInt(job.notification.try) > 0)
        notification.try_max = parseInt(job.notification.try);
    if (notification.try_max < 1)
        notification.try_max = 1;
    if (notification.try_max > config.jobs.notifications.try_max)
        notification.try_max = config.jobs.notifications.try_max;
    if (job.notification?.retry_in && parseInt(job.notification.retry_in) > 0)
        notification.retry_in = parseInt(job.notification.retry_in);
    if (notification.retry_in < (1 * 1000))
        notification.retry_in = (1 * 1000);
    if (notification.retry_in > config.jobs.notifications.retry_in_max)
        notification.retry_in = config.jobs.notifications.retry_in_max;
...
export async function retryJobNotification(notification) {
...
export async function notify(specs, payload) {
    let outcome = {
        status: 'FAILED',
    };
    try {
...
async function notifyHttp(specs, payload) {
    ...
    if (timeout < 1)
        timeout = 1;
    if (timeout > config.jobs.notifications.timeout_max)
        timeout = config.jobs.notifications.timeout_max;
    const requestConfig = {
        method,
        headers,
        timeout,
    ...
    if (response.status !== 200)
        outcome.error = { message: response.statusText || 'Unknown error occurred!' };
    if (response.headers)
        outcome.header = response.headers;
    if (response.data)
        outcome.body = response.data;
    return outcome;
}
async function notifySns(specs, payload) {
    ...
    const command = new PublishCommand({
        ...
        Subject: `Job ${payload?.status || 'update'}`,
    });
    ...
    if (response.$metadata.httpStatusCode !== 200)
        outcome.error = { message: 'Unknown error occurred!' }; // response.$metadata?.httpStatusText
    if (response.MessageId)
        outcome.message_id = response.MessageId;
    return outcome;
}
