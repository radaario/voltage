...
const jobDestinationS3LikeSchema = Joi.object({
	...
	region: Joi.string().required(),
	bucket: Joi.string().required(),
	force_path_style: Joi.boolean().default(false).failover(false),
	...
	public_url_base: Joi.string().uri().failover(null).allow(null)
});
...
const jobOutputConfigCommonFileSchema = {
	acl: Joi.string().constantcase().validOrFallback(STORAGE_S3_LIKE_ACLS, STORAGE_S3_LIKE_ACLS[0]).failover(STORAGE_S3_LIKE_ACLS[0]),
	expires_in: Joi.number().failover(null).allow(null),
	cache_control: Joi.string().failover(null).allow(null)
};
...
const jobOutputConfigThumbnailSchema = Joi.object({
	...
	format: Joi.string().constantcase().validOrFallback(THUMBNAIL_FORMATS, THUMBNAIL_FORMATS[0]).failover(THUMBNAIL_FORMATS[0]).required(),
	...jobOutputConfigCommonSchema,
	offset: Joi.number().range(0, undefined).failover(0),
...
const jobOutputConfigSubtitleSchema = Joi.object({
	...
	format: Joi.string().constantcase().validOrFallback(SUBTITLE_FORMATS, SUBTITLE_FORMATS[0]).failover(SUBTITLE_FORMATS[0]).required(),
	...jobOutputConfigCommonSchema,
	offset: Joi.number().range(0, undefined).failover(0),
	duration: Joi.number().range(1, undefined).failover(null).allow(null),
