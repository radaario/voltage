...
const Logs: React.FC = () => {
	...
	const { data: logsResponse, isLoading } = useQuery<ApiResponse<Log[]>>({
		queryKey: ["output-logs", output.key, currentPage, currentLimit, searchQuery, typeFilter, authToken],
		queryFn: async () => {
						...
						<div className="flex items-center justify-end sm:justify-start gap-2">
							<Tooltip content="View">
								<Button
									...
									onClick={(e) => {
										e.stopPropagation();
										navigate(`/jobs/${output.job_key}/outputs/${output.key}/logs/${log.key}/info`);
									}}>
