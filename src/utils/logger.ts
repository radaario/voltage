...
class Logger {
  ...
  async insert(type: string, message: string, metadata: any = {}) {
    ...
    const log = { key: uukey(), type, instance_key, worker_key, job_key, output_key, notification_key, message: this.sanitizeMessage(message, metadata), metadata: metadata ? JSON.stringify(metadata) : null, created_at: getNow() };
